...
evaluated experimentally. For the evaluation, we use a subset of
...
Our configuration for running experiments was as follows:
- \sbt{} timeout: =120s=
...
3. There is too many trivial \Phi nodes inserted. Too many trivial \Phi
   nodes also cause our implementation to run out of memory.

We were interested in why our implementation is slower than the
original one. Multiple measurements show that our implementation
inserts too many trivial \Phi nodes. For our measurements, we count how
many \Phi nodes are inserted by our implementation. We also count how
many of them are /trivial/ -- they have at most one operand or
multiple operands which are the same variable. The results of our
measurements can be seen in Figure\nbsp{}\ref{fig:trivialPhiPercent}.
...
our implementation to compute unnecessary information.
...
the attached =dg.zip=. This guide is targeted for Ubuntu system of
version at least 16.04.
