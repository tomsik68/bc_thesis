...
and discusses the simplest naive algorithm to compute reaching
...
node of the CFG: $IN_v$ and $OUT_v$. $IN_v$ and $OUT_v$ contain incoming
or outgoing reaching definitions respectively. Both of them are
mappings of variables to CFG nodes. Figure \ref{fig:denseRDA}
demonstrates the algorithm.

To translate the semantics of instructions into something an
...
$x$ is a definition. $KILL_x$ is set of variables that
are overwritten in this node. The $KILL$ and $GEN$ sets do *not*
...
conservative assumptions about the program to be correct. This
sub-chapter describes three properties of reaching definitions
analyses: instance-wiseness, field sensitivity and ability to
...
a\nbsp{}strong definition, the definition at \encircle{1} would be overwritten
by the definition at \encircle{2}, because they were allocated by the
...
LLVM defines its intermediate representation (LLVM IR) of a program. The
...
Before the reaching definitions analysis itself, DG builds a
...
1. In the first phase, the implementation constructs a /sparse RD
   ...
   =readVariable=). When a \Phi node $y$ is created as a result, add an
...
the whole range of the variable is not overwritten. This issue is
...
significant differences. The new subgraph builder splits up LLVM basic
blocks when a function call is encountered, and it also adds
information about which memory is used in which CFG node. The
following two sections discuss these additions.
...
needed by Braun et al. algorithm -- that is =current_def= and
=current_weak_def=. This way, the field-sensitivity is considered in
the phase of building the sparse RD graph.
...
definition of an unknown offset of a variable, it could be a definition
...
framework introduced in\nbsp{}Section \ref{chap:intervals}, the new implementation
...
new analysis is slower than the original one. The new implementation
turned out to be more accurate than the original one in some cases.
...
This part of the guide has been tested on a fresh installation of
Ubuntu 18.04 inside a Docker container.
...
#+BEGIN_SRC sh
  ...
  $ llvm-link "test1.sliced" "test_assert.bc" -o "test1_linked.bc"
#+END_SRC
...
When running \sbt{}, command line parameter
SRC_sh[]{--slicer-params="-rda ss"} instructs the slicer to use the new
