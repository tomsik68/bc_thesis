...
Several formal verification tools allow us to detect such
errors. Symbiotic is one of them. Symbiotic preprocesses the program
in a specific way and then passes it to symbolic execution framework
klee which performs the verification itself. 

As formal verification by itself is an expensive process\cite{_}, it
is suitable to remove parts of the program that do not influence
result of the verification process. This is called program
slicing. Reaching definitions analysis is used during slicing to
determine whether each instruction of the program could be removed.

The aim of this thesis is to study some techniques of computing
reaching definitions, implement a faster analysis to Symbiotic and
compare the new implementation with the original one.
